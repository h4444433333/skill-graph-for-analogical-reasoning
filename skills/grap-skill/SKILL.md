...
The GitHub/source version and the ClawHub/OpenClaw skill version should stay
linked:

- GitHub/source users install the Python package and run `grap-skill` directly
- skill users install `skill-graph-for-analogical-reasoning` through OpenClaw

The command examples in the published skill bundle use `python3 {baseDir}/scripts/run_grap_skill.py`
because that wrapper is the stable entrypoint intended for model-driven skill
execution inside the installed bundle.
